...
leinstein(true); /* Einstein tensor with lower indices, display=true */
\end{listing}
Note that our definition of \verb@phit@ omits the factor of 2 from the line element, because we're
using it to set two different off-diagonal elements of the metric, and we don't want to double-count.
% petrov_curvature.mac

The output of \verb@leinstein@ is silent, despite our setting the display flag to \verb@true@; this is because
$G_{ab}$'s elements are all zero, and the function only outputs any nonzero elements. We can verify this by
explicitly asking for output of the components \verb@lein[1,1]@, etc.

Maxima computes the scalar curvature $R$ and outputs it, but isn't smart enough to simplify it completely.
It is smart enough, however, to show us that it does not depend on $t$, $\phi$, or $z$.
A good way to get around this problem is to expand $R$ in a Taylor series as a function of $r$.
For example, a call to \verb@taylor(scurvature(),r,0,12)@ shows that the first 12 terms of the
Taylor series all vanish.
