...
\index{covariant vector!summarized}\index{vectors (covariant and contravariant)!summarized}
...
Galilean spacetime and mechanical systems described with generalized coordinates
...
Derivatives with respect to the coordinates can be used as a basis for the space of
covariant vectors. 
But in a nearly universal abuse of notation, one uses the derivative operators as a
basis for the space of contravariant vectors, e.g., writing contravariant vector $\vc{v}$ in terms of
its components $v^\mu$ and the basis of covariant vectors $\partial_\mu$ as $\vc{v}=v^\mu \partial_\mu$.
This satisfies our desire to see up-down pairs of indices in the Einstein summation notation, but
violates the convention that no-index quantities should be scalars and, more importantly,
writes a contravariant vector illogically as the sum of covariant vectors. For similar reasons,
one sees covariant vectors written as sums of contravariant basis vectors, e.g.,
$\boldsymbol{\omega}=\omega_\mu \der x^\mu$.
\end{document}
